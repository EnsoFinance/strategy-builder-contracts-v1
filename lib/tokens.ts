...
export class Tokens {
  ...
  async registerTokens(owner: SignerWithAddress, strategyFactory: Contract, uniswapV3Registry?: Contract, chainlinkRegistry?: Contract, curveDepositZapRegistry?: Contract) {
    await Promise.all([
      ...
      strategyFactory.connect(owner).addItemToRegistry(ITEM_CATEGORY.CLAIMABLE, ESTIMATOR_CATEGORY.CURVE_GAUGE, this.crvUSDPGauge),
      strategyFactory.connect(owner).addItemToRegistry(ITEM_CATEGORY.CLAIMABLE, ESTIMATOR_CATEGORY.CURVE_GAUGE, this.crvAAVEGauge),
			strategyFactory.connect(owner).addItemToRegistry(ITEM_CATEGORY.CLAIMABLE, ESTIMATOR_CATEGORY.CURVE_GAUGE, this.crvSAAVEGauge),
			strategyFactory.connect(owner).addItemToRegistry(ITEM_CATEGORY.CLAIMABLE, ESTIMATOR_CATEGORY.CURVE_GAUGE, this.crvCOMPGauge),
      strategyFactory.connect(owner).addItemToRegistry(ITEM_CATEGORY.CLAIMABLE, ESTIMATOR_CATEGORY.CURVE_GAUGE, this.crvLINKGauge),

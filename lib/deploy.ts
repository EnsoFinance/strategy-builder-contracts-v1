...
export async function deployPlatform(
	...
	*/
	...
	const uniswapV2LPEstimator = await waffle.deployContract(owner, UniswapV2LPEstimator, [])
...
export async function deployEnsoToken(
	owner: SignerWithAddress,
	minter: SignerWithAddress,
	name: string,
	symbol: string,
	mintingAllowedAfter: number
): Promise<Contract> {
...
export async function deployEnsoEstimator(
	owner: SignerWithAddress,
	sEnso: Contract,
...
export async function deployStakedEnsoEstimator(
	owner: SignerWithAddress,
	strategyFactory: Contract
): Promise<Contract> {
	...
	await strategyFactory
		.connect(owner)
		.addEstimatorToRegistry(ESTIMATOR_CATEGORY.ENSO_STAKED, stakedEnsoEstimator.address)
...
export async function deployEnsoStakingAdapter(
	owner: SignerWithAddress,
	staking: Contract,
	...
	const adapter = await waffle.deployContract(owner, EnsoStakingAdapter, [
		staking.address,
...
export async function deployUniswapV2LPAdapter(
	owner: SignerWithAddress,
	uniswapV2Factory: Contract,
	weth: Contract
): Promise<Contract> {

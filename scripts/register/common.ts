...
import { CrawlerOutput, uniV3HighestFeePools } from "./jsonData";
import ERC20 from '@uniswap/v2-periphery/build/ERC20.json'
import { ITEM_CATEGORY, ESTIMATOR_CATEGORY } from "../../lib/constants";

// FIXME combine with ../common
...
export function findSupportedV3Pair(token0: string, token1: string): V3LiquidityInfo {
  const pairs = uniV3HighestFeePools.filter(pool => { return isPair(token0, pool.token, token1, pool.pair) })
...
export function findSupportedV3Tokens(token0: string, token1: string): boolean {
  const token0Support = uniV3HighestFeePools.findIndex(pool => pool.token.toLowerCase() === token0.toLowerCase() || pool.pair.toLowerCase() === token0.toLowerCase()) !== -1
  const token1Support = uniV3HighestFeePools.findIndex(pool => pool.token.toLowerCase() === token1.toLowerCase() || pool.pair.toLowerCase() === token1.toLowerCase()) !== -1
...
export interface V2LiquidityInfo extends AmmPair {
  wethValue: number;
}

export interface V3RegistryInfo extends TokenPair {
  liquidity: string;
  fee: BigNumber;
};

export interface V3LiquidityInfo extends AmmPair {
  liquidity: string;
  wethValue: number;
  fee: BigNumber;
};

export interface V3LiquidityInfoJson extends AmmPair {
  liquidity: string;
  wethValue: number;
  fee: number;
};
...
export interface V3LiquidityInfoDictionary {
  [key: string]: V3LiquidityInfo
}

...
import { ESTIMATOR_CATEGORY } from '../lib/constants'
...
export class Deployer {
	...
	constructor(signer: SignerWithAddress, network: string, overwrite: boolean) {
		this.signer = signer
	...
	async deploy(contractName: string, params: any[], libraries?: { [key: string]: string }): Promise<Contract> {
		const actualContractName = contractAliases[contractName] || contractName
	...
	async setupEstimators(uniswapOracleAddress: string, chainlinkOracleAddress: string) {
		let contractHandle: Contract
		let fnPtr: (estimatorCategory: number, estimatorAddress: string, txArgs: TransactionArgs) => void = (
		...
		await this.setupEstimatorsWithFunc(
			uniswapOracleAddress,
			chainlinkOracleAddress,
			async (estimatorCategory: number, estimatorAddress: string) => {
	...
	async setupEstimatorsWithFunc(
		...
		await this.setupEstimator(ESTIMATOR_CATEGORY.STRATEGY, 'StrategyEstimator', [], registerEstimator)
		await this.setupEstimator(ESTIMATOR_CATEGORY.BLOCKED, 'EmergencyEstimator', [], registerEstimator)
		await this.setupEstimator(ESTIMATOR_CATEGORY.AAVE_V2, 'AaveV2Estimator', [], registerEstimator)
		// this.add2Deployments('AaveEstimator', aaveV2Estimator.address) //Alias
		await this.setupEstimator(ESTIMATOR_CATEGORY.AAVE_V2_DEBT, 'AaveV2DebtEstimator', [], registerEstimator)
		// this.add2Deployments('AaveDebtEstimator', aaveV2DebtEstimator.address) //Alias
		await this.setupEstimator(ESTIMATOR_CATEGORY.COMPOUND, 'CompoundEstimator', [], registerEstimator)
		await this.setupEstimator(ESTIMATOR_CATEGORY.CURVE_LP, 'CuveLPEstimator', [], registerEstimator)
		// this.add2Deployments('CurveEstimator', curveLPEstimator.address) //Alias
		await this.setupEstimator(ESTIMATOR_CATEGORY.CURVE_GAUGE, 'CurveGaugeEstimator', [], registerEstimator)
		await this.setupEstimator(ESTIMATOR_CATEGORY.YEARN_V2, 'YEarnV2Estimator', [], registerEstimator)
	}
	...
	async setupEstimator(
		...
		const estimatorAddress = await this.deployOrGetAddress(contractName, params)
	...
	write2File() {
		...
		require('fs').writeFileSync('./deployments.json', data)
	}
...
export const getExpectedBaseFee = (block: any) => {
	...
	if (block.baseFeePerGas) {
		...
		if (block.gasUsed.gt(target)) {
			...
			expectedBaseFee = block.baseFeePerGas.add(
				block.baseFeePerGas.mul(1000).div(8).mul(diff).div(target).div(1000)
			)
		} else {
			const diff = target.sub(block.gasUsed)
			expectedBaseFee = block.baseFeePerGas.sub(
				block.baseFeePerGas.mul(1000).div(8).mul(diff).div(target).div(1000)
			)
		}
	}

	...
	before('Setup Uniswap + Factory', async function () {
		...
		for (var i = 0; i < strategies.length; ++i) {
			...
			for (var j = 0; j < items.length; ++j) {
				...
				if (found) {
					await strategies[i]
						.connect(await impersonate(await strategies[i].manager()))
		...
		const newStrategyFactoryImplementation = await waffle.deployContract(accounts[0], StrategyProxyFactory, [
			controller.address,
		])

...
const { deployUniswap, deployPlatform, deployUniswapRouter, deployGenericController } = require('./helpers/deploy.js')
...
describe('GenericController', function () {
    let tokens, accounts, uniswapFactory, portfolioFactory, oracle, genericController, router, portfolio, portfolioTokens, portfolioPercentages, portfolioRouters, wrapper, whitelist
    ...
    before('Setup Uniswap, Factory, GenericController', async function() {
      ...
      [portfolioFactory, oracle, whitelist] = await deployPlatform(accounts[0], genericController, uniswapFactory, WETH);
    })
    ...
    it('Should rebalance portfolio with multicall', async function () {
        ...
        /*
        const delegateSwapSig = genericController.interface.getSighash('_delegateSwap')
        console.log('_delegateSwap: ', delegateSwapSig)
        const settleSwapSig = genericController.interface.getSighash('_settleSwap')
        console.log('_settleSwap: ', settleSwapSig)
        const settleTransferSig = genericController.interface.getSighash('_settleTransfer')
        console.log('_settleTransfer: ', settleTransferSig)
        */
        ...
        //const data = ethers.utils.defaultAbiCoder.encode(['bytes[]'], [calls])
        const data = genericController.encodeCalls(calls);

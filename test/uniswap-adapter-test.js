...
describe('UniswapAdapter', function () {
    ...
    it('Should fail to swap: tokens cannot match', async function() {
      await expect(adapter.swap(1, 0, tokens[0].address, tokens[0].address, accounts[0].address, accounts[0].address, '0x', '0x')).to.be.revertedWith('tokenIn and tokenOut cannot match')
    })

    it('Should fail to swap: no msg.value for token swap', async function() {
      await expect(adapter.swap(1, 0, tokens[0].address, tokens[1].address, accounts[0].address, accounts[0].address, '0x', '0x', {value: 1})).to.be.revertedWith('Cannot send value if tokenIn is not Ether')
    })

    it('Should fail to swap: insufficent received', async function() {
      await expect(adapter.swap(10, 1000, AddressZero, tokens[1].address, accounts[0].address, accounts[0].address, '0x', '0x', {value: 10})).to.be.revertedWith('Insufficient tokenOut amount')
    })

    it('Should swap eth for token', async function() {
      await adapter.swap(100000, 0, AddressZero, tokens[1].address, accounts[0].address, accounts[0].address, '0x', '0x', {value: 100000})
    ...
    it('Should swap token for token', async function() {
      await adapter.swap(token1Balance, 0, tokens[1].address, tokens[0].address, accounts[0].address, accounts[0].address, '0x', '0x')

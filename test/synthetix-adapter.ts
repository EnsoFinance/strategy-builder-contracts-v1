...
describe('SynthetixAdapter', function () {
	...
	before('Setup Synthetix, Uniswap, Curve, Enso', async function () {
		...
		//crvsusd
		await curvePoolRegistry.addPool(
		...
		await chainlinkOracle.connect(accounts[0]).addOracle(tokens.susd, MAINNET_ADDRESSES.WETH, '0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419', true);
		await chainlinkOracle.connect(accounts[0]).addOracle(tokens.seur, tokens.susd, '0xb49f677943BC038e9857d61E7d053CaA2C1734C1', false);
		...
		curveAdapter = await deployCurveAdapter(accounts[0], curvePoolRegistry, weth)
		await whitelist.connect(accounts[0]).approve(curveAdapter.address)
		synthetixAdapter = await deploySynthetixAdapter(accounts[0], resolver, weth)
	...
	it('Should deploy strategy', async function () {
		...
		const positions = [
			...
			{ token: tokens.susd, percentage: BigNumber.from(300), adapters: [uniswapAdapter.address, curveAdapter.address], path: [tokens.usdc] },
			{ token: tokens.seur, percentage: BigNumber.from(300), adapters: [synthetixAdapter.address], path: [] }
		]
	...
	it('Should withdraw synths into reserve', async function () {
		...
		await strategy.connect(accounts[1]).settleSynths(synthetixAdapter.address, susd.address);
		//await displayBalances(wrapper, strategyItems.map((item) => item.item), weth)
	})

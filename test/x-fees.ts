...
describe('StrategyToken Fees', function () {
	...
	it('Should deposit', async function () {
		...
		const tx = await controller.connect(manager).deposit(strategy.address, router.address, 0, DEFAULT_DEPOSIT_SLIPPAGE, '0x', { value: BigNumber.from('10000000000000000') })
		const receipt = await tx.wait()

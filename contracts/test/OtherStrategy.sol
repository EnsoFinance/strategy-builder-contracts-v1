...
import "./OtherStrategyStorage.sol";
 ...
 */
contract OtherStrategy is IStrategy, IStrategyManagement, OtherStrategyStorage, StrategyCommon, Initializable, Timelocks, Require {
    ...
    function initialize(
        ...
        _symbol = symbol_;
        _version = version_;
        DEPRECATED_DOMAIN_SEPARATOR = bytes32(uint256(-1)); // disallows migration
    ...
    function migrateDeprecated() external {
        require(DEPRECATED_DOMAIN_SEPARATOR != bytes32(uint256(-1)), "already migrated.");
        // where did the deprecated storage go? see `StrategyStorage`
        _token.migrateDeprecated(
            DEPRECATED_DOMAIN_SEPARATOR,
            DEPRECATED_streamingFeeRate,
            DEPRECATED_performanceFee,
            DEPRECATED_lastStreamTimestamp,
            DEPRECATED_lastTokenValue,
            DEPRECATED_managementFee,
    ...
    function _setItem(StrategyItem memory strategyItem, ITokenRegistry tokenRegistry) private returns(int256) {
        ...
        if (category < ItemCategory.RESERVE) {
